...
const generateConditionFeedback = (
  ...
  if (nutrient === 'sodium_mg' && status === 'high') {
    return '🧂 Too much salt! This might not be ideal for heart health. Try using less next time.';
  }
...
const ResultModal: React.FC<ResultModalProps> = ({ score, customer, feedback, onNext, isLastRound, aiCoachSuggestion, tip }) => {
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col animate-jump-in">
        ...
        <div className="p-6 text-center overflow-y-auto flex-1">
            ...
            <div className="mt-4 p-3 bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  <p className="text-xs font-bold text-blue-900 mb-1">AI Coach</p>
                  <p className="text-sm text-blue-800 leading-relaxed">{aiCoachSuggestion}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {/* Did You Know Tip */}
          {tip && (
            <div className="mt-4 p-3 bg-amber-50 border border-amber-100 rounded-lg text-left">
              ...
              {tip.source_name && (
                <p className="text-[10px] text-amber-700 mt-1 text-right">
                  Source: {tip.source_url ? <a href={tip.source_url} target="_blank" rel="noopener noreferrer" className="underline">{tip.source_name}</a> : tip.source_name}
                </p>
              )}
            </div>
          )}
